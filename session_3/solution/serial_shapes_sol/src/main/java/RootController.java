...
public class RootController {
    ...
    public void saveShapes() {
        SerialDeserial.serial(shapeArrayList);
    }

    public void loadShapes() {
        shapeArrayList = SerialDeserial.deserial();

        if (shapeArrayList != null) {
            int largestID = 0;
            for (Shape shape : shapeArrayList) {
                if (shape.getUniqueID() > largestID) {largestID = shape.getUniqueID();}
            }
            ShapeFactory.setShapeCounter(largestID);
            ComboBoxFactory.setCurrentShapes(shapeArrayList);
        }
    }
